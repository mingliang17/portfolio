...
const ZoomModal = ({ image, onClose, magnifySize, zoomFactor }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    setMagnify(prev => ({ ...prev, x: x + rect.left, y: y + rect.top,
...
const CarouselSection = ({ carouselData = [], title = 'Gallery', showControls = true, showIndicators = true }) => {
  ...
  const handleMouseEnter = (index) => {
    if (index !== currentIndex || zoomedImage) return;
    const startTime = Date.now();
    const duration = 2000;
    const animateRing = () => {
      const progress = Math.min((Date.now() - startTime) / duration, 1);
      setHoverProgress(prev => ({ ...prev, [index]: progress }));
      if (progress < 1) {
        hoverTimersRef.current[index] = requestAnimationFrame(animateRing);
      } else {
        setZoomedImage(data[index].image);
  ...
  const handleMouseLeave = (index) => {
    if (hoverTimersRef.current[index]) {
      cancelAnimationFrame(hoverTimersRef.current[index]);
      delete hoverTimersRef.current[index];
    }
    setHoverProgress(prev => ({ ...prev, [index]: 0 }));
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Object.values(hoverTimersRef.current).forEach(t => cancelAnimationFrame(t));
    };
  }, [zoomedImage, move]);
            ...
            {data.map((item, i) => {
              ...
              return (
                <div key={item.id} className="carousel-item" style={getItemStyle(i)}
                  ...
                  {/* ── Ring glow is OUTSIDE the image wrapper, on the carousel-item directly ── */}
                  {isCenter && progress > 0 && (
                    <div
                      className="hover-ring-glow"
                      style={{
                        background: `conic-gradient(
                          from -90deg,
                          rgba(100, 149, 237, 0.9) 0deg,
                          rgba(120, 200, 255, 1) ${progress * 360}deg,
                          rgba(100, 149, 237, 0) ${progress * 360}deg,
                          rgba(100, 149, 237, 0) 360deg
                        )`,
                      }}
                    />
                  )}

                  {/* ── Actual image (clipped, ring glow doesn't affect it) ── */}
        ...
        <div className="bottom-panel">
          ...
          {showIndicators && total > 1 && (
            <div className="indicators">
              {data.map((_, i) => (
                <button key={i} className={`indicator ${i === currentIndex ? 'active' : ''}`}
